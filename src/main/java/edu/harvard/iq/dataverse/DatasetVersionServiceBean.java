...
public class DatasetVersionServiceBean implements java.io.Serializable {
    ...
    public class RetrieveDatasetVersionResponse {
        ...
        public RetrieveDatasetVersionResponse(DatasetVersion datasetVersion, String requestedVersion) {
            ...
            // logger.fine("RetrieveDatasetVersionResponse: datasetVersion: " +
            // datasetVersion.getSemanticVersion() + " requestedVersion: " +
            // requestedVersion);
            // logger.fine("chosenVersion id: " + datasetVersion.getId() + "
            // getFriendlyVersionNumber: " + datasetVersion.getFriendlyVersionNumber());
        ...
        public String getDifferentVersionMessage() {
            ...
            if (this.wasSpecificVersionRequested && !this.didSpecificVersionMatch) {
                ...
                if (DatasetVersionServiceBean.this.isVersionAskingForDraft(this.requestedVersion)) {
                    ...
                    userMsg = BundleUtil.getStringFromBundle("file.viewDiffDialog.msg.versionNotFound",
                            Arrays.asList(MarkupChecker.escapeHtml(this.requestedVersion)));
                }
                ...
                if (DatasetVersionServiceBean.this.isVersionAskingForDraft(this.actualVersion)) {
                    ...
                    userMsg += BundleUtil.getStringFromBundle("file.viewDiffDialog.msg.versionFound",
                            Arrays.asList(this.actualVersion));
                }
        ...
        private void checkVersion() {
            ...
            // logger.fine("check version. requested: " + this.requestedVersion + "
            // returned: " + actualVersion);
    ...
    /**
     * Performs the same database lookup as the one behind
     * {@code Dataset.getVersions()}.
     * <p>
     * Additionally, supports:
     * <ul>
     * <li>Pagination via {@code offset} and {@code length}</li>
     * <li>Filtering by visibility (all, released only, or released +
     * deaccessioned)</li>
     * </ul>
     * <p>
     ...
     * @param includeAllVersions   if {@code true}, retrieves all versions (drafts,
     *                             released, and deaccessioned)
    ...
    public DatasetVersion findByFriendlyVersionNumber(Long datasetId, String friendlyVersionNumber) {
        ...
        if (majorVersionNumber != null && minorVersionNumber == null) {
            try {
                TypedQuery<DatasetVersion> typedQuery = em.createQuery(
                        "SELECT v from DatasetVersion v where v.dataset.id = :datasetId  and v.versionNumber= :majorVersionNumber",
    ...
    public boolean isVersionDefaultCustomTerms(DatasetVersion datasetVersion) {
        // SEK - belt and suspenders here, but this is where the bug 10719 first
        // manifested
        if (datasetVersion != null && datasetVersion.getId() != null) {
            try {
                TermsOfUseAndAccess toua = (TermsOfUseAndAccess) em
                        .createNamedQuery("TermsOfUseAndAccess.findByDatasetVersionIdAndDefaultTerms")
                        .setParameter("id", datasetVersion.getId())
                        .setParameter("defaultTerms", TermsOfUseAndAccess.DEFAULT_NOTERMS).getSingleResult();
    ...
    public List<DatasetVersionUser> getDatasetVersionUsersByAuthenticatedUser(AuthenticatedUser user) {
        ...
        TypedQuery<DatasetVersionUser> typedQuery = em.createQuery(
                "SELECT u from DatasetVersionUser u where u.authenticatedUser.id = :authenticatedUserId",
     ...
     * 
     * @param identifierClause - query clause to retrieve via DatasetVersion.Id or
     *                         DatasetVersion.persistentId
     ...
     * @return String fullQuery
     */
    private String getNumericDatasetVersionQueryByIdentifier(String identifierClause, Long majorVersion,
            Long minorVersion) {
        ...
        if (majorVersion != null && minorVersion != null) {
            ...
            extraQueryClause += " AND (dv.versionstate = '" + VersionState.RELEASED.toString()
                    + "' or dv.versionstate = '" + VersionState.DEACCESSIONED.toString() + "')";
        } else {
    ...
    /**
     * @deprecated because of a typo; use
     *             {@link #retrieveDatasetVersionByIdentifierClause(String, String)
     *             retrieveDatasetVersionByIdentifierClause} instead
    ...
    public DatasetVersion retrieveDatasetVersionByIdentifierClause(String identifierClause, String version) {
        ...
        if (versionNumbers != null && versionNumbers.length == 2) { // At least a major version found
            ...
            String specificVersionQuery = this.getNumericDatasetVersionQueryByIdentifier(identifierClause,
    ...
    public RetrieveDatasetVersionResponse selectRequestedVersion(List<DatasetVersion> versions, String versionTag) {
        ...
        for (DatasetVersion version : versions) {
            ...
            if (this.isVersionAskingForDraft(versionTag)) {
                ...
                if (majorVersion.equals(version.getVersionNumber())
                        && minorVersion.equals(version.getMinorVersionNumber())
                        && (version.isReleased() || version.isDeaccessioned())) {
                    return new RetrieveDatasetVersionResponse(version, versionTag);
                }
    ...
    public Long getThumbnailByVersionId(Long versionId) {
        ...
        if (!FeatureFlags.DISABLE_DATASET_THUMBNAIL_AUTOSELECT.enabled()) {
            ...
            if (thumbnailFileId != null) {
                logger.fine(
                        "DatasetVersionService,getThumbnailByVersionid(): found already generated thumbnail for version "
                                + versionId + ": " + thumbnailFileId);
    ...
    private void assignDatasetThumbnailByNativeQuery(Long versionId, Long dataFileId) {
        try {
            em.createNativeQuery("UPDATE dataset SET thumbnailfile_id=" + dataFileId
                    + " WHERE id in (SELECT dataset_id FROM datasetversion WHERE id=" + versionId + ")")
                    .executeUpdate();
        } catch (Exception ex) {
    ...
    public void writeEditVersionLog(DatasetVersionDifference dvd, AuthenticatedUser au) {
        ...
        String summary = au.getFirstName() + " " + au.getLastName() + " (" + au.getIdentifier() + ") updated "
                + dvd.getEditSummaryForLog();
    ...
    public void populateDatasetSearchCard(SolrSearchResult solrSearchResult) {
        String parentIdStr = solrSearchResult.getParent() != null ? solrSearchResult.getParent().get("id") : null;
        if (parentIdStr == null || "0".equals(parentIdStr) || "".equals(parentIdStr)) {
            return;
        }
        Long dataverseId = Long.parseLong(parentIdStr);
        ...
        try {
            if (datasetId != null) {
                searchResult = (Object[]) em.createNativeQuery(
                        "SELECT t0.VERSIONSTATE, t1.ALIAS, t2.THUMBNAILFILE_ID, t2.USEGENERICTHUMBNAIL, t3.STORAGEIDENTIFIER FROM DATASETVERSION t0, DATAVERSE t1, DATASET t2, DVOBJECT t3 WHERE t0.ID = "
                                + datasetVersionId
                                + " AND t1.ID = "
                                + dataverseId
                                + " AND t2.ID = "
                ...
                searchResult = (Object[]) em.createNativeQuery(
                        "SELECT t0.VERSIONSTATE, t1.ALIAS FROM DATASETVERSION t0, DATAVERSE t1 WHERE t0.ID = "
                                + datasetVersionId + " AND t1.ID = " + dataverseId)
                        .getSingleResult();
            }
        } catch (Exception ex) {
            return;
        ...
        if (searchResult.length == 5) {
            ...
            try {
                globalId = PidUtil.parseAsGlobalID(globalIdentifier);
            } catch (IllegalArgumentException e) {
                logger.warning(
                        "PidUtil.parseAsGlobalID threw exception for: " + globalIdentifier + " - " + e.getMessage());
            }
            ...
            if (globalId != null) {
                ...
                logger.warning("Could not parse GlobalId from: " + globalIdentifier);
                // Fallback: try to split manually or just set as much as we can
                if (globalIdentifier != null && globalIdentifier.contains(":")) {
                    String[] parts = globalIdentifier.split(":", 2);
                    datasetEntity.setProtocol(parts[0]);
                    if (parts[1].contains(":")) {
                        String[] subParts = parts[1].split(":", 2);
                        datasetEntity.setAuthority(subParts[0]);
                        datasetEntity.setIdentifier(subParts[1]);
                    } else {
                        datasetEntity.setIdentifier(parts[1]);
                    }
                }
            }
    ...
    /**
     * Return a list of the checksum Strings for files in the specified
     * DatasetVersion
     * 
    ...
    /**
     * Check for the existence of a single checksum value within a DatasetVersion's
     * files
     * 
     * @param datasetVersion
    ...
    public HashMap getFileMetadataHistory(DataFile df) {
        ...
        List<String> colsToRetrieve = Arrays.asList("df.id", "df.contenttype", "df.filesize", "df.checksumtype",
                "df.checksumvalue", "fm.label", "fm.description", "fm.version");
    ...
    public JsonObjectBuilder fixMissingUnf(String datasetVersionId, boolean forceRecalculate) {
        ...
        if (!StringUtils.isBlank(datasetVersion.getUNF())) {
            info.add("message", "Dataset version (id=" + datasetVersionId
                    + ") already has a UNF. Blank the UNF value in the database if you must change it.");
            return info;
        }
        ...
        if (fileUnfsInVersion.isEmpty()) {
            info.add("message", "Dataset version (id=" + datasetVersionId
                    + ") has no tabular data files with UNF signatures. The version UNF will remain blank.");
            return info;
        }
        ...
        if (!forceRecalculate) {
            ...
            if (previousDatasetVersion != null) {
                ...
                if (isFileUnfsIdentical(fileUnfsInVersion, fileUnfsInPreviousVersion)) {
                    ...
                    info.add("message", "Dataset version (id=" + datasetVersionId
                            + ") has the same tabular file UNFs as a previous version. Assigned the UNF of the previous version without recalculation ("
                            + previousDatasetVersion.getUNF()
                            + "). Use the --forceRecalculate option if you insist on recalculating this UNF.");
                }
            }
        }
    ...
    public List<DatasetVersion> getUnarchivedDatasetVersions() {
        ...
        try {
            List<DatasetVersion> dsl = em
                    .createNamedQuery("DatasetVersion.findUnarchivedReleasedVersion", DatasetVersion.class)
                    .getResultList();
            return dsl;
        } catch (NoResultException e) {
     ...
     *
     * @param datasetId                  The unique identifier of the dataset for
     *                                   which to count versions.
     *                                   Must not be {@code null}.
     * @param canViewUnpublishedVersions A boolean flag that controls the scope of
     *                                   the count:
     *                                   <ul>
     *                                   <li>{@code true} - All versions of the
     *                                   dataset are counted,
     *                                   regardless of their
     *                                   {@link VersionState}.</li>
     *                                   <li>{@code false} - Only versions with a
     *                                   state of
     *                                   {@link VersionState#RELEASED} or
     *                                   {@link VersionState#DEACCESSIONED} are
     *                                   counted.</li>
     *                                   </ul>
     * @return A {@code Long} representing the total count of matching dataset
     *         versions.
     *         This will be {@code 0L} if the dataset has no versions or does not
     *         exist.
     */
