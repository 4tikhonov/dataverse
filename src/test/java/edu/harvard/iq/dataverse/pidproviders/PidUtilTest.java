...
public class PidUtilTest {
    ...
    public static void setUpClass() throws Exception {
        ...
        // This mimics the initial config in the
        // PidProviderFactoryBean.loadProviderFactories method - could potentially be
        // used to mock that bean at some point
    ...
    public void testGetDoi() throws IOException {
        ...
        GlobalId pid = new GlobalId(AbstractDOIProvider.DOI_PROTOCOL, "10.70122", "QE5A-XN55", "/",
    ...
    public void testPermaLinkGenerationiWithSeparator() throws IOException {
        ...
        System.out.println(
                "Provider prefix for perma identifier" + p.getAuthority() + p.getSeparator() + p.getShoulder());
        assertTrue(
                ds.getGlobalId().asRawIdentifier().startsWith(p.getAuthority() + p.getSeparator() + p.getShoulder()));
    }
    ...
    public void testDOIParsing() throws IOException {
        ...
        assertEquals("https://doi.org/" + pid2.getAuthority()
                + PidUtil.getPidProvider(pid2.getProviderId()).getSeparator() + pid2.getIdentifier(), pid2.asURL());
        ...
        // Lower case is recognized by converting to upper case internally, so we need
        // to test vs. the upper case identifier
        // I.e. we are verifying that the lower case string is parsed the same as the
        // upper case string, both give an internal upper case PID representation
    ...
    public void testHandleParsing() throws IOException {
        ...
        assertEquals(
                "https://hdl.handle.net/" + pid2.getAuthority()
                        + PidUtil.getPidProvider(pid2.getProviderId()).getSeparator() + pid2.getIdentifier(),
                pid2.asURL());
        ...
        // Test case sensitivity - a handle with uppercase "TEST" should not match the
        // hdl1 provider
    ...
    public void testUnmanagedParsing() throws IOException {
        ...
        assertEquals(
                HandlePidProvider.HDL_RESOLVER_URL + pid2.getAuthority()
                        + PidUtil.getPidProvider(pid2.getProviderId()).getSeparator() + pid2.getIdentifier(),
                pid2.asURL());
    ...
    public void testFindingPidGenerators() throws IOException {
        ...
        String pidGeneratorSpecs = Json.createObjectBuilder().add("protocol", AbstractDOIProvider.DOI_PROTOCOL)
                .add("authority", "10.5072").add("shoulder", "FK2").build().toString();
        ...
        // Change dataset to have a provider and verify that it is used instead of any
        // effective one
        ...
        assertTrue(PidUtil.getPidProvider(dataset1.getEffectivePidGenerator().getId())
                .canCreatePidsLike(dataset1.getGlobalId()));

        dataset1.setPidGenerator(null);
        // Now set identifier so that the provider has this one in it's managed list
        // (and therefore we can't mint new PIDs in the same auth/shoulder) and
        // therefore we get the effective pid generator
        ...
        dataset1.setPidGenerator(null);
        pidGeneratorSpecs = Json.createObjectBuilder().add("protocol", AbstractDOIProvider.DOI_PROTOCOL)
                .add("authority", "10.9999").add("shoulder", "FK2").build().toString();
        // Set a PID generator on the parent
        dataverse1.setPidGeneratorSpecs(pidGeneratorSpecs);
        assertEquals(pidGeneratorSpecs, dataverse1.getPidGeneratorSpecs());
        // Verify that the parent's PID generator is the effective one
    ...
    public void testLegacyPermaConfig() throws IOException {
        MockitoAnnotations.openMocks(this);
        Mockito.when(settingsServiceBean.getValueForKey(SettingsServiceBean.Key.Shoulder)).thenReturn("FK2");
        Mockito.when(settingsServiceBean.getValueForKey(SettingsServiceBean.Key.Protocol))
                .thenReturn(PermaLinkPidProvider.PERMA_PROTOCOL);
